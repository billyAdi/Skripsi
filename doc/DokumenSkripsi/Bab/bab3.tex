...
\item Program melakukan migrasi \textit{database}.
...
Pada proyek Piktora terdapat 58 \textit{commmit}. Setelah dilakukan pengujian terdapat beberapa masalah. Masalah tersebut yaitu perbedaan letak \textit{file}, migrasi \textit{database}, dan konfigurasi \textit{database}. Masalah-masalah tersebut dibahas pada subbab \ref{subsec:perbedaan_letak_file} sampai \ref{subsec:migrasi_database}.

\subsection{Perbedaan Letak \textit{File}}
\label{subsec:perbedaan_letak_file}
Pada \textit{commit} ke-1 sampai ke-38 halaman \textit{web} pada proyek Piktora tidak bisa dibuka. Hal ini disebabkan oleh perbedaan letak \textit{file} "index.php". Pada \textit{commit} ke-1 sampai ke-38 \textit{file} "index.php" berada pada \textit{folder} "www", sedangkan pada \textit{commit} ke-39 sampai ke-58 \textit{file} "index.php" berada pada \textit{folder} "httpdocs".

Solusi untuk mengatasi masalah ini adalah dengan menambahkan \textit{command line options} yang menerima sebuah \textit{script} PHP. \textit{Script} PHP ini mengecek letak \textit{file} "index.php" pada \textit{folder} "www" dan "httpdocs". \textit{Script} kemudian mengecek \textit{directory root} XAMPP pada \textit{file} "httpd.conf".  Jika \textit{directory root} sudah mengarah ke \textit{folder} tempat "index.php" berada, maka \textit{script} tidak mengubah isi \textit{file} "httpd.conf". Jika \textit{directory root} tidak mengarah ke \textit{folder} tempat "index.php" berada, maka \textit{script} akan mengubah \textit{directory root} pada \textit{file} "httpd.conf". Setelah \textit{script} menjalankan perintah "httpd -k restart", untuk melakukan \textit{restart} pada XAMPP. \textit{Script} PHP ini dijalankan sebelum melakukan migrasi \textit{database}.   


\subsection{Permasalahan Konfigurasi \textit{Database}}
\label{subsec:konfigurasi_database}
Pada \textit{commit} ke-39 sampai ke-41, halaman \textit{web} tidak bisa dibuka. Hal ini disebabkan karena perbedaan konfigurasi pada \textit{file} "database.php". Pada \textit{commit} 39 s.d. 46 dan \textit{commit} 49 terdapat \textit{password} pada \textit{file} "database.php". \textit{Commit} lainnya tidak terdapat \textit{password} pada \textit{file} "database.php". Penulis menggunakan \textit{password} "piktora" pada konfigurasi \textit{database} di XAMPP.

\begin{figure}[H]
	\centering
		\includegraphics[scale=0.5]{Gambar/piktora_commit_39.png}
	\caption{\textit{Commit} ke-39 pada proyek Piktora}
	\label{fig:piktora_commit_39}
\end{figure} 

Gambar \ref{fig:piktora_commit_39} merupakan isi dari \textit{file} "database.php" pada \textit{commit} ke-39.
Dapat dilihat bahwa \textit{password} yang terdapat pada \textit{file} "database.php" adalah "dmHx64\%6". Pada  \ref{fig:piktora_commit_42} \textit{password} untuk \textit{database} adalah "piktora". \textit{Commit} 39 s.d. 41 menggunakan \textit{password} "dmHx64\%6", sedangkan \textit{commit} 42 s.d.46 dan 48 menggunakan \textit{password} "piktora". Karena konfigurasi \textit{password} pada \textit{file} "database.php" dan XAMPP berbeda, halaman \textit{website} pada \textit{commit} 39 s.d. 41 tidak bisa dibuka. 

\begin{figure}[H]
	\centering
		\includegraphics[scale=0.5]{Gambar/piktora_commit_42.png}
	\caption{\textit{Commit} ke-42 pada proyek Piktora}
	\label{fig:piktora_commit_42}
\end{figure} 

Sama seperti subbab \ref{subsec:perbedaan_letak_file}, solusi untuk mengatasi masalah ini adalah dengan menambahkan \textit{command line options} yang menerima sebuah \textit{script} PHP. \textit{Script} PHP inilah yang mengubah \textit{password} dari \textit{file} "database.php". \textit{Script} PHP dijalankan pada program dengan menggunakan kelas Runtime dan Process yang terdapat pada library Java. \textit{Script} dijalankan sebelum melakukan migrasi \textit{database}. 

Cara kerja dari \textit{Script} PHP yang dimasukkan ke \textit{command line options} adalah sebagai berikut:
\begin{enumerate}
\item Mengisialisasi \textit{path} dari \textit{file} "database.php"
\item Memvalidasi \textit{path} menggunakan. Jika \textit{path} tidak valid \textit{script} selesai dijalankan.
\item Mendapatkan semua isi \textit{file} dan menampungnya ke suatu variabel, misal "\$contents".
\item Mencari \textit{string password} menggunakan \textit{regular expression}.
\item Jika tidak terdapat \textit{password} atau terdapat \textit{password} berupa "piktora", isi \textit{file}tidak diubah dan \textit{script} selesai dijalankan.
\item Jika terdapat \textit{password} berupa "dmHx64\%6", \textit{string password} pada variabel "\$contents" diubah menjadi "piktora".
\item Melakukan \textit{overwrite} pada \textit{file} "database.php" dengan variabel "\$contents".   
\end{enumerate}
...
Pada \textit{commit} 24, terjadi \textit{error} saat melakukan migrasi database. Pada \textit{commit} 23, terdapat satu \textit{file} untuk melakukan migrasi yaitu "20161122150000\_Structure.php". Pada \textit{commit} 24 terdapat dua \textit{file} untuk melakukan migrasi yaitu "20161122150000\_Structure.php" dan "20161122150001\_InitialData.php". Pada \textit{commit} 23 \textit{file} "20161122150001\_InitialData.php" dijalankan saat melakukan migrasi. Versi migrasi \textit{database} menjadi "20161122150000". Pada \textit{commit} 24, \textit{file} "20161122150000\_Structure.php" tidak dijalankan karena dianggap sama dengan versi migrasi \textit{database} saat ini. Hanya \textit{file} "20161122150001\_InitialData.php" yang dijalankan pada \textit{commit} 24. Isi \textit{file} "20161122150001\_InitialData.php" pada \textit{commit} 23 dan 24 berbeda. Hal ini yang menyebabkan terjadinya \textit{error} saat melakukan migrasi database.  

Sama seperti subbab \ref{subsec:perbedaan_letak_file}, solusi untuk mengatasi masalah ini adalah dengan menambahkan \textit{command line options} yang menerima sebuah \textit{script} PHP. \textit{Script} ini melakukan dua pekerjaan. Pertama, \textit{script} menghapus \textit{database} piktora dengan menggunakan \textit{query} "DROP DATABASE piktora". Setelah itu \textit{script} membuat \textit{database} piktora dengan menggunakan \textit{query} "CREATE DATABASE piktora". \textit{Script} dijalankan sebelum melakukan migrasi \textit{database}. 
