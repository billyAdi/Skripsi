...
WebDriver merupakan representasi dari \textit{browser} yang digunakan untuk \textit{automation testing}. WebDriver merupakan suatu \textit{interface}. WebDriver dapat diinisialisasi menggunakan \textit{object} dengan tipe ChromeDriver. Setelah melakukan inisialisasi pada WebDriver, \textit{browser} akan dijalankan. Untuk membuka suatu halaman \textit{web}, digunakan \textit{method} get() dengan parameter alamat URL. Gambar \ref{fig:webdriver} menunjukkan sebuah \textit{browser} yang dikontrol oleh ChromeDriver. 
