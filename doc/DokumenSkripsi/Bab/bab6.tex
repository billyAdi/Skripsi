...
\begin{itemize}
\item Animasi \textit{timelapse} pada pengembangan proyek perangkat lunak berbasis \textit{web} dapat dibangkitkan dengan bantuan Git. Dengan Git, bisa didapatkan halaman-halaman \textit{web} dari proyek perangkat lunak berbasis web, kemudian \textit{screenshot} dari halaman-halaman tersebut digabung menjadi satu \textit{file} bertipe GIF.
...
\begin{enumerate}
\item Saat ini \textit{output} dari program berupa \textit{file} bertipe GIF. \textit{File} bertipe GIF ini memiliki dua kekurangan, yaitu keterbatasan warna dan ukuran \textit{file} yang besar. GIF memakai sistem palet warna dan hanya terbatas pada 256 warna saja. Selain itu, \textit{file} hasil animasi dari situs \textit{web} Bootstrap berukuran 160 MB. Dalam pengembangan berikutnya, sebaiknya menggunakan format \textit{file} yang lebih modern seperti MP4 atau format lainnya. 
