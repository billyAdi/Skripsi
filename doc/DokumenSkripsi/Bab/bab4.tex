...
Kelas ini berfungsi untuk menyimpan semua \textit{option} yang terdapat dalam program ini, dan melakukan \textit{parsing} argumen Command Line Options yang dimasukkan oleh \textit{user}.  
...
Merupakan Constructor dari kelas ini. Berfungsi untuk menentukan \textit{option} yang terdapat pada program dan melakukan parsing argumen Command Line. Parameternya adalah argumen Command Line Option yang didapatkan dari
kelas Main.
...
Constructor dari kelas ini. Berfungsi untuk menginisialisasi variabel git dan mendapatkan seluruh histori commit  proyek perangkat lunak berbasis web pada branch tertentu. Dimana branch tersebut diambil dari parameter constructor. Parameternya adalah \textit{path} dari proyek perangkat lunak berbasis web dan nama \textit{branch} yang digunakan untuk membangkitkan animasi.
...
Berfungsi untuk membangkitkan animasi \textit{timelapse} berdasarkan langkah-langkah pada subbab \ref{sec:analisis_fitur_aplikasi}. Hasil animasi berupa \textit{file} bertipe GIF. Parameternya adalah objek yang bertipe VCS, BrowserController, dan Properties. Parameter properties menampung key dan value dari \textit{option} yang sudah diparsing. 
\end{itemize}
...
Pesan ini muncul jika \textit{path} pada argumen \textit{option} \texttt{project-path} tidak valid.
