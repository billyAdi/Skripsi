...
Kelas ini berfungsi untuk menyimpan semua \textit{option} yang terdapat dalam program ini, dan melakukan \textit{parsing} argumen Command Line yang dimasukkan oleh \textit{user}.  
...
Constructor dari kelas ini. Berfungsi untuk menginisialisasi variabel git dan mendapatkan seluruh histori \textit{commit} dari repositori perangkat lunak berbasis \textit{web} pada \textit{branch} tertentu. Dimana branch tersebut diambil dari parameter constructor. Parameternya adalah \textit{path} dari repositori perangkat lunak berbasis web dan nama \textit{branch} yang digunakan untuk membangkitkan animasi.
