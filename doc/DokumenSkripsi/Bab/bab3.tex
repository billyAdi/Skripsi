...
%belum beres 
Program pada skripsi ini memiliki dua fitur. Fitur utama dari program ini adalah membangkitkan animasi timelapse. Selain itu, perangkat lunak dapat membaca masukan dan konfigurasi dari Command Line Options yang dimasukkan oleh \textit{user}. Penjelasan fitur-fitur pada perangkat lunak dapat dilihat pada Gambar \ref{fig:uc}, Tabel \ref{tab:tabel_sc_input} , dan Tabel \ref{tab:tabel_sc_animasi}.


\begin{figure}[H]
	\centering
		\includegraphics[scale=0.5]{Gambar/UseCaseDiagram1.png}
...
\begin{table}[]
    ...
    \hline
        Nama & Memasukkan \textit{input} dan konfigurasi\\
    \hline
    \hline
        Deskripsi & Fitur untuk memasukkan \textit{input} dan konfigurasi perangkat lunak\\
    \hline
        Aktor & \textit{User} \\
    \hline
        Pre-kondisi & Xampp sudah dijalankan \\
    \hline
        Pos-kondisi & Input berhasil dimasukkan \\
    \hline
        Skenario & 
        \begin{enumerate}
            \item \textit{User} memasukan \textit{input} dan konfigurasi perangkat lunak melalui Command Line Options.
            \item Sistem membaca masukan dari \textit{user}.
        \end{enumerate}\\
    \hline
    \end{tabular}
    \caption{\textit{Scenario case} memasukkan \textit{input}}
    \label{tab:tabel_sc_input}
\end{table}
...
\begin{table}[]
    ...
    \hline
        Aktor & \textit{User} \\
    \hline
        Pre-kondisi & \textit{Input} dan konfigurasi sudah dimasukkan \\
    \hline
